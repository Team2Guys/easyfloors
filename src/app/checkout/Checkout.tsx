                                    ...
                                    {mergedCart.length > 0 ? mergedCart.map((item, index) => (
                                        <div key={index} className="flex items-center border-b pb-4">
                                            ...
                                            <div className="ml-4">
                                                ...
                                                {item.isfreeSample ? "" :
                                                    <p className="text-sm text-gray-600 text-12 xs:text-14">{item.category === "Accessories" ? <>Total Required: <span className="font-semibold">{item.requiredBoxes}</span></> : <>No. of Boxes: <span className="font-semibold">{item.requiredBoxes}</span> ({item.squareMeter.toFixed(2)} SQM)</>}</p>
                                                }
                                                <p className="md:text-sm text-gray-600 text-12">{item.category?.toLowerCase() === 'accessories' ? "Piece Price" : "Box Price"}: <span className="font-currency text-15 font-normal"></span>{item.pricePerBox}</p>
