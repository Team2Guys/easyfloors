...
import { orderSummary } from "data/bin/bin";

const ThankYouComp: React.FC = () => {
    return (
        ...
        <div className="max-w-4xl mx-auto md:p-0 p-2">
            ...
            <div className="flex flex-col justify-center items-center gap-4 md:my-16 my-6">
                <Link href="/accessories" className="bg-primary text-white max-w-fit md:px-28 px-8 md:py-4 py-2 flex items-center md:text-lg text-sm font-inter font-light md:gap-3 gap-1"> <IoIosArrowRoundBack className="text-32" />  Back to Shopping</Link>
            ...
            <div className="bg-[#FFF9F5]  ">
                <div className="border-b md:p-7 p-3 ">
                    <h2 className="md:text-3xl text-xl font-inter">Order Summary <sup className="md:text-sm md:ml-3 text-10 text-red-500">(*Total 3 Items)</sup> </h2>
                </div>              <div className="md:p-10 p-2">
                    <div className="max-h-72 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
                        {orderSummary.map((item: OrderItem, index: number) => (
                            <div key={index} className="md:pr-6">
                                <div className="flex items-center justify-between border-b gap-5 pb-4 mb-4">
                                    <div className="flex items-center md:gap-5 gap-2">
                                        <Image src={item.image} width={100} height={100} alt={item.title} className="md:w-20 md:h-20 w-16 h-16 object-cover border p-1" />
                                        <div>
                                            <p className="font-inter font-bold md:text-base text-sm">{item.title}</p>
                                            <p className="md:text-sm text-gray-600 mt-1 text-12">No. Of Boxes: {item.boxes} ({item.sqm} SQM)</p>
                                        </div>
                                    </div>
                    ...
                    <div className="mt-6 text-right">
                        <div className="flex justify-between items-center">
                            <p className="text-[#818EA1] text-lg">Subtotal</p>
                            <p className="text-lg font-medium"> AED 1050.00</p>
                        </div>
                        <div className="flex items-center gap-4 mt-4">
                            ...
                            <p className="lg:text-xl text-lg font-bold whitespace-nowrap">AED 1102.50</p>
                        </div>
