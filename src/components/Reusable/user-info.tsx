      ...
      {HomeUserInfo.map((section, index) => (
        ...
        >
          <div className='text-center flex-1 '>
            ...
            <Image width={100} height={100} className='w-[24px] h-[24px] md:w-[57px] md:h-[57px]' src={section.icon} alt={section.title}  />
              <span className=' text-16 md:text-26 2xl:text-[34px] font-semibold'>{section.title}</span>
              <p className='mt-2 text-10 lg:text-16 2xl:text-20 font-light'>{section.description}</p>
            </div>
          </div>
          <div className='flex-1'>
            <Image width={848} height={501} src={section.image} alt={section.title}  />
          </div>
        </div>
      ))}
    </Container>
