...
const Breadcrumb = ({ title, image = "", slug, subcategory }: BreadcrumbProps) => {
  return (
    <div className="relative w-full">
      ...
      <div className={`flex flex-col px-4 font-inter ${!image ? "bg-background text-black py-1" : "text-white absolute inset-0 justify-center text-center items-center "}`}>
        ...
        {title && (
          <nav className="md:mt-2 text-lg flex items-center gap-2 font-bold">
            ...
            <svg width="7" height="12" viewBox="0 0 7 12"  className={`${!image ? "text-black" : "text-white"}`} xmlns="http://www.w3.org/2000/svg">
              <path
            ...
            {slug && subcategory ? (
              <>
                <Link href={`/${slug}`} className="hover:underline text-12 md:text-14">
                  {slug}
                </Link>
                <svg width="7" height="12" viewBox="0 0 7 12" className={`${!image ? "text-black" : "text-white"}`} xmlns="http://www.w3.org/2000/svg">
                  <path
                ...
                <Link href={`/${slug}/${subcategory}`} className="hover:underline text-12 md:text-14">
                  {subcategory}
                </Link>
                <svg width="7" height="12" viewBox="0 0 7 12" className={`${!image ? "text-black" : "text-white"}`} xmlns="http://www.w3.org/2000/svg">
                  <path
