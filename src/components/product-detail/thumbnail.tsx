...
const Thumbnail = ({ ThumnailImage, ThumnailBottom, hideThumnailBottom = false, imageheight, onImageChange }: ExtendedThumbnailProps) => {
  ...
  const combinedImages = useMemo(() => [...ThumnailImage, ...(ThumnailBottom || [])], [ThumnailImage, ThumnailBottom]);
          ...
          {ThumnailImage.map((product, index) => (
            ...
            >
              <Image
                ...
                className={`w-full ${imageheight ? "h-[35px] sm:h-[73px] md:h-[230px]" : "h-[35px] sm:h-[73px] md:h-[124px]"}`}
          ...
          {combinedImages.map((product, index) => (
            <div key={index} className={`relative ${product.plankWidth && 'py-1 sm:py-0'}`}>
              <Image
                ...
                height={800}
                src={product.imageUrl}
                className={`w-full ${imageheight ? "h-[273px] sm:h-[520px] md:h-[1218px]" : "h-[273px] sm:h-[520px] md:h-[830px]"}`}
                alt={product.altText || 'Thumbnail'}
              />
              {product.plankHeight && (
                <div className="absolute h-full top-0 flex flex-col justify-between py-4 sm:py-10 left-1/2 -translate-x-28 sm:-translate-x-36">
                  <span className="flex justify-center items-center"><FaAngleUp className="text-20 sm:text-24" /></span>
                  <div className="flex-1 border w-[1px] mx-auto border-black"></div>
                  <span className="h-16 sm:h-28 flex justify-center items-center font-semibold transform rotate-90 text-13 sm:text-16">{product.plankHeight}</span>
                  <div className="flex-1 border w-[1px] mx-auto border-black"></div>
                  <span className="flex justify-center items-center"><FaAngleDown className="text-20 sm:text-24" /></span>
                </div>
              )}
              {product.plankWidth && (
                <div className="absolute w-[170px] sm:w-[200px] -top-1 sm:top-0 flex justify-between mx-auto left-1/2 -translate-x-24 sm:-translate-x-28">
                  <span className="flex justify-center items-center transform -rotate-90"><FaAngleUp className="text-20 sm:text-24" /></span>
                  <div className="flex-1 border h-[1px] my-auto border-black"></div>
                  <span className="flex justify-center items-center text-13 sm:text-16 font-semibold px-2">{product.plankWidth}</span>
                  <div className="flex-1 border h-[1px] my-auto border-black"></div>
                  <span className="flex justify-center items-center transform -rotate-90"><FaAngleDown className="text-20 sm:text-24" /></span>
                </div>
              )}
            </div>
          ))}
              ...
              return (
                ...
                >
                  <Image
                    ...
                    src={array.imageUrl}
                    alt={array.altText}
                    className={`w-full max-sm:h-[39px] object-contain ${globalIndex === currentSlide ? "shadow-xl" : ""}`}
                  />
