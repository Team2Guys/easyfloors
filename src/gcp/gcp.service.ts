...
export class GoogleMerchantService {
  ...
  async uploadProducts(merchantId: string) {
    const authClient = this.authService.getOAuthClient();
    ...
    for (const product of products) {
      ...
      if(product.subcategory){
        url = url+product.category?.RecallUrl+'/'+product.subcategory.custom_url+"/"+product.custom_url

      }
      url = url = url+product.category?.RecallUrl+'/'+product.custom_url
      try {
        const res = await content.products.insert({
          ...
          requestBody: {
            ...
            // optional: add more fields like brand, gtin, mpn, etc.
          },
        });
